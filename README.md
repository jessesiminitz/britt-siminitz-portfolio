...
- **`data/articles.json`** — the bio plus hand-picked "Selected Work"
  highlights, each with an excerpt. Curated by hand.
- **`data/jck_archive.json`** — the *complete* JCK archive (3,680 articles,
  2012–2026), generated by `scripts/scrape_jck_archive.py`. Powers the
  searchable archive section. Don't hand-edit; re-run the scraper instead:

  ```bash
  python3 scripts/scrape_jck_archive.py
  ```

  JCK's author page paginates through a WordPress `admin-ajax` endpoint
  (`action=more_writers&author=6`) rather than normal page URLs — the
  script drives that directly, 100 records per request with a 0.5s delay.
  Re-run it periodically to pick up new articles.
...
Every article in the curated "Selected Work" section was individually
fetched and its byline verified before being added — nothing there is
guessed or paraphrased from a search snippet. The full JCK archive comes
straight from JCK's own author-archive endpoint, so it is authoritative
by construction.
